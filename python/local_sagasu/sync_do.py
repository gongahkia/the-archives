...
def check_file(filepath=".env"):
    """
    checks whether the specified file exists within the current folder
    """
    if os.path.isfile(filepath):
        print(f"File '{filepath}' exists in the current repository.")
        return True
    else:
        print(f"Error: File '{filepath}' does not exist in the current repository.")
        return False
...
def read_config():
    """
    read scraping configuration from a .env file
    """
    load_dotenv()
    date_raw = str(os.getenv("DATE_RAW"))
    duration_hrs = float(os.getenv("DURATION_HRS"))
    start_time = str(os.getenv("START_TIME"))
    room_capacity_raw = int(os.getenv("ROOM_CAPACITY_RAW"))
    building_array = [each.strip() for each in str(os.getenv("BUILDING_ARRAY")).split(",")]
    floor_array = [each.strip() for each in str(os.getenv("FLOOR_ARRAY")).split(",")]
    facility_type_array = [each.strip() for each in str(os.getenv("FACILITY_TYPE_ARRAY")).split(",")]
    equipment_array = [each.strip() for each in str(os.getenv("EQUIPMENT_ARRAY")).split(",")]
    if date_raw and duration_hrs and start_time and room_capacity_raw and building_array and floor_array and facility_type_array and equipment_array:
        return {
            "DATE_RAW": date_raw,
            "DURATION_HRS": duration_hrs,
            "START_TIME": start_time,
            "ROOM_CAPACITY_RAW": room_capacity_raw,
        ...
        print("One or more scraping configurations are missing in the .env file")
...
def scrape_smu_fbs(base_url, screenshot_filepath, booking_log_filepath, date_raw, duration_hrs, start_time, room_capacity_raw, building_array, floor_array, facility_type_array, equipment_array):
    ...
    START_TIME = local_config["START_TIME"]
    ROOM_CAPACITY_RAW = local_config["ROOM_CAPACITY_RAW"]
    BUILDING_ARRAY = local_config["BUILDING_ARRAY"]
    FLOOR_ARRAY = local_config["FLOOR_ARRAY"]
    FACILITY_TYPE_ARRAY = local_config["FACILITY_TYPE_ARRAY"]
    EQUIPMENT_ARRAY = local_config["EQUIPMENT_ARRAY"]
